...
public class Robot extends SampleRobot {
	...
	//PSYCH THESE ARE ALL FLIPPED BOI
	//LEFT IN CODE IS RIGHT IRL & RIGHT IN CODE IS LEFT IRL
	
	SpeedController shooter = new Spark(4); //For Shooter
	
	SpeedController winch = new Spark(5); //For Winch
	
	//SpeedController feeder = new Talon(6);
	...
	JoystickButton trigger = new JoystickButton(leftStick, 1); //ShooterButton
	
	//JoystickButton feederButtonForward = new JoystickButton(rightStick, 3);
	//JoystickButton feederButtonBackward = new JoystickButton(rightStick, 4 );
	...
	public void operatorControl() {
		...
		while (isOperatorControl() && isEnabled()) {
			 ...
			 */
			 if(rightStick.getY() >= 0){
			  	winch.set(Math.pow(rightStick.getY(), 2));
			 ...
			 //Shooter Feeder
			 //if(feederButtonForward.get()){
				// feeder.set(1);
			 //}else if(feederButtonBackward.get()){
				// feeder.set(-1);
			 //}else{
				// feeder.set(0);
			 //}
	...
	public void autonomous() {
		...
		case turnAutoRightBumper:
			...
			break;
		case turnAndDriveRightBumperAuto: //RED ALLIANCE
			myRobot.setSafetyEnabled(false);
			frontLeft.set(-.7);
			backLeft.set(-.7);
			frontRight.set(0);
			...
			frontLeft.set(-.6); //Need to have different ratios so we don't run into the wall, I have no idea why this is
			backLeft.set(-.6);
			frontRight.set(.45);
			backRight.set(.45);
			...
			break;
		case turnAndDriveLeftBumperAuto:  //BLUE ALLIANCE
			myRobot.setSafetyEnabled(false);
			frontLeft.set(0);
			backLeft.set(0);
			frontRight.set(.7);
			...
			case shootTurnDriveLeftBumperAuto://BLUE ALLIANCE 
			myRobot.setSafetyEnabled(false); //Turn left forward then both forward
			...
			frontLeft.set(-.5);	//Need to have different ratios so we don't run into the wall, I have no idea why this is
			backLeft.set(-.5);
			frontRight.set(.6);
			backRight.set(.6);
		...
		case shootTurnDriveRightBumperAuto: //RED ALLIANCE
			myRobot.setSafetyEnabled(false); //Right forward then both forward
			myRobot.drive(0.0, 0.0);
			shooter.set(.9);
			//feeder.set(-1);
			Timer.delay(10);
